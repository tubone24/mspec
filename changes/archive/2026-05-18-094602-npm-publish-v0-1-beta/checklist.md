...
- [x] **FR-003 — ベータバージョンでの npm tag 管理**: `--tag beta` なしの `npm install -g @mspec/cli` では `latest` タグが汚染されず、beta バージョンがデフォルトでインストールされないこと <!-- verify: fr-003 -->
...
- [x] **FR-003 — GitHub Release トリガーによる publish**: GitHub UI または `gh release create` で Release を作成し `release: released` イベントで GitHub Actions が起動、npm publish が実行されること <!-- verify: fr-003 -->
