...
5. For each Lesson bullet:
   - **Concreteness detection (step 5-pre)**: Determine if the Lesson contains any of the following signals:
     - Tool names (e.g. `mspec`, `git`, `npm`, `node`, `docker` — proper nouns for CLIs/tools)
     - Command names (e.g. `mspec new`, `mspec continue`, `git commit` — verb+tool combinations)
     - File names (strings containing a file extension: `.md`, `.ts`, `.json`, `.yaml`, etc.)
     - File paths (strings containing `/` or `./` path notation)
     - If ANY signal is present → **abstraction required** (see `text` rule below)
     - If NO signal is present → **pass-through** (use the Lesson text as-is for `text`)
   ...
   - If not duplicated, generate an abstracted proposal with:
     - `text`:
       - **Abstraction required**: Remove concreteness signals (tool/file names, command syntax). Rewrite as a process principle or design lesson in one sentence. Maximum one level of abstraction — do NOT escalate to philosophical generalities. Example: "mspec validate を実行し忘れた" → "成果物の完了条件を事前に明示することで品質ゲートを担保する"
       - **Pass-through**: Use the Lesson text unchanged (already an abstract principle)
